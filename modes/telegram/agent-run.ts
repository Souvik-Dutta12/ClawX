...
const agentOptions = (config: AgentConfig, maxSteps: number) => {
    return {
        model: getAgentModel(),
...
const createReadOnlyTools = (executor: ToolExecutor) => {
    return {
        read_file: tool({
...
const extraWebTools = (tracker: ActionTracker) => {
...
export async function runPlanSteps(
    ...
    await finishOrApprove(ctx, chatId, tracker, executor, 'All steps done. No file changes were needed.');
  }