...
const isProbablyTextFile = (filePath: string): boolean => {
...
export class ToolExecutor {
    ...
    searchFiles(rootRel: string, globPattern: string, contentQuery?: string): string {
        ...
        const walk = (dir: string) => {
            for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
                ...
                if (ent.isDirectory()) {
                    ...
                    if (contentQuery && !isProbablyTextFile(full)) continue;

                    const text = fs.readFileSync(full, "utf8");
                    if (contentQuery && !text.includes(contentQuery)) continue;
        ...
        if (fs.statSync(rootAbs).isDirectory()) {
            walk(rootAbs);
        } else if (nameRe.test(path.basename(rootAbs))) {
            const relp = path.relative(this.config.codebasePath, rootAbs)
                .split(path.sep)
                .join("/");

            if (!contentQuery || (isProbablyTextFile(rootAbs) &&
                fs.readFileSync(rootAbs, "utf8").includes(contentQuery))) {
                results.push(relp);
            }
        }
    ...
    skillRoots(): string[] {
        const extra = process.env.SKILL_DIRS?.split(/[;]/)
            .map((s) => s.trim())
            .filter(Boolean) ?? [];
        return [
            ...extra,
    ...
    applyApprovedFromTracker(): { errors: string[] } {
        ...
        for (const a of all.filter(
            (a) => a.type === "folder_create" && a.status === "approved"
        )) {
            try {
